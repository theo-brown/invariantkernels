...
class GroupInvariantKernel(gpytorch.kernels.Kernel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Use lazy evaluations throughout (ie __call__ rather than forward)
        # This means some torch operations aren't possible (e.g. .sum((-3, -4)) -> .sum(-3).sum(-3))
        ...
        # Shape (..., N)
        K_x1x1 = (
            self.base_kernel(x1_orbits_transpose, x1_orbits_transpose).sum(-1).sum(-1)
        )
        # Shape (..., M)
        K_x2x2 = (
            self.base_kernel(x2_orbits_transpose, x2_orbits_transpose).sum(-1).sum(-1)
        )
        # Shape (..., N, M)
